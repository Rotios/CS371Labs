...
vec3 texture(sampler2D channel, vec3 p)
{
    vec3 ta = texture2D(channel, p.xy).xyz;
    vec3 tb = texture2D(channel, p.xz).xyz;
    vec3 tc = texture2D(channel, p.yz).xyz;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 o = vec3(0.0, 0.0, iGlobalTime * 4.0);
    o.x -= sin(o.z * ss);
    o.y -= cos(o.z * ss);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        vec3 stex = texture(iChannel1, sw * 0.5);
        sk += stex * fr * 0.25;
    }
    ...
    prod = pow(prod, 4.0);
    vec3 pcol = texture(iChannel2, w * 0.1) * prod;
