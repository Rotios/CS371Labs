...
vec3 GetSunColor(vec3 rayDir, vec3 sunDir)
{
	...
	localRay.x = localRay.x + 1.0 - iGlobalTime * 0.1;
	...
	float blink = texture2D(iChannel2, localRay.xy + iGlobalTime * 0.03).x;
...
vec3 LensFlare(vec2 uv, vec2 lfPos)
{
	...
	vec3 tex = texture2D(iChannel0, vec2(angle*5.0, dist*0.125) /*- iGlobalTime*0.1*/).xyz;
...
vec2 Spiral(vec2 uv)
{
	...
	//offset *= dist;
	return uv + offset*vec2(1.0,1.0)*1.1*texBlurry.x ;//+ vec2(iGlobalTime*0.03, 0.0);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 iA, iB, iA2, iB2;
	float t = iGlobalTime * 0.1 + 0.7 - iMouse.x*0.01;
	float cloudT = iGlobalTime * 0.1;
...
/*	if (abs(normal.x) < abs(normal.z))
	...
	//vec3 sunDir = normalize(vec3(-0.9 + sin(iGlobalTime*0.2)*0.5, -0.1, -0.9150));
	vec3 sunDir = normalize(vec3(-0.009 + sin(iGlobalTime*orbitSpeed), -0.13, -cos(iGlobalTime*orbitSpeed)));
	vec3 r = normalize(cross(sunDir, vec3(0.0,1.0,0.0)));
	vec3 up = normalize(cross(sunDir, r));
	float binarySpeed = 0.5;
	float binaryDist = 0.3;
	sunDir += r * sin(iGlobalTime*binarySpeed) * binaryDist + up * cos(iGlobalTime*binarySpeed) * binaryDist;
	sunDir = normalize(sunDir);

	vec3 sunDir2 = normalize(vec3(-0.009 + sin((iGlobalTime+0.2)*orbitSpeed), 0.13, -cos((iGlobalTime+0.2)*orbitSpeed)));
	r = normalize(cross(sunDir2, vec3(0.0,1.0,0.0)));
	up = normalize(cross(sunDir2, r));
	sunDir2 -= r * sin(iGlobalTime*binarySpeed) * binaryDist + up * cos(iGlobalTime*binarySpeed) * binaryDist;
