...
bool Planet::readSpec(const Any& planetSpec) {
    try {
        ...
        x.getIfPresent("numberOfClouds", m_numberOfClouds);
        x.getIfPresent("hasBirds", m_hasBirds);
        x.getIfPresent("numberOfBirds", m_numberOfBirds);
...
bool Planet::generatePlanet() {
    try {
        ...
        findTreePositions(landMapImage, vertices, m_treePositions);
...
void Planet::addCloudEntityToPlanet(Any& cloudEntity, const String& modelName, const Point3& position, const float orbitAngle, const float orbitSpeed) {
    addFlyingEntityToPlanet(cloudEntity, modelName, position, orbitAngle, orbitSpeed);
...
void Planet::addFlyingEntityToPlanet(Any& entity, const String& modelName, const Point3& position, const float orbitAngle, const float orbitSpeed) {
    String track;
    ...
    track = (String)
        ...
        CoordinateFrame::fromYAxis((position - m_position).unit(), (position + (position - m_position).unit() * (Random::threadCommon().uniform(6, 11)))*m_scale).toXYZYPRDegreesString() + "))";

    entity["track"] = Any::parse(track);
}
...
void Planet::createEntityModelAnyFile(Any& model, const String& name, const String& fileName, const String& preprocess) {
...
void Planet::getTreePositions(Array<Vector3>& vertices) {
    vertices = m_treePositions;
}

void Planet::findTreePositions(const shared_ptr<Image>& landMap, const Array<Vector3>& vertices, Array<Vector3>& positions) {
...
int Planet::getNumberOfClouds(){
    return m_numberOfClouds;
}
